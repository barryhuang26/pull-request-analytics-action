...
| `EXCLUDE_DIFF_LINE_PATTERNS`| Regular expressions for diff lines that should be ignored when calculating additions and deletions. Values should be separated by commas. Use `/pattern/flags` if flags are needed. 例如：`/^\\s*$/` 忽略空白行。系統會預設排除含有 `github.com/dudoo-team/` 的行。                                                                                                                                | -                                                                       |
